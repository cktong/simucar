...
Initial Values:     7,5,10,8,.7,1,1,2,1,7,7,7,7,3,1,1,1,1,15,15,15,15,500,30,1,7,10000,15000,20000,25000,350,400,450,500,50
...
long   ARRMM[5][2];	/***  Arrival min and max number of arrivals per day  ***/
...
int  _edge_condition[4];
  ...
  if (_edge_condition[0])
    ...
    event_time = current_time + (1/ARRMM[J][1])+RND*(1/ARRMM[J][0]-(1/ARRMM[J][1]));
...
int  _edge_condition[2];
  ...
  if (_edge_condition[0])
    ...
    event_time = current_time + REN[J][0]+(REN[J][1]-REN[J][0])*BET(1.2,1.2);
  ...
  if (_edge_condition[1])
    ...
    event_time = current_time + FAILVAR;
...
int  _edge_condition[1];
  ...
  /* Evaluate edge conditions now so that they will*/
  /* not be changed by preemptive event execution  */
  _edge_condition[0] = ( ENT[3]/5000+ENT[4]>7.2&&S[J]>0 );
