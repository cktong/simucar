...
Model Name:         carrent1201.mod
Model Description:  A DETERMINISTIC QUEUE (M/D/1) WITH BREAKDOWNS
Output File:        untitled.out
Output Plot Style:  NOAUTO_FIT
Run Mode:           HI_SPEED
Trace Vars:         L[1],L[2],L[3],L[4],L1[1],L1[2],L1[3],L1[4],L2[1],L2[2],L2[3],L2[4],SERVICED[1],SERVICED[2],SERVICED[3],SERVICED[4],INFLOW[1],INFLOW[2],INFLOW[3],INFLOW[4],CUMCOST[1],CUMCOST[2],CUMCOST[3],CUMCOST[4],NETP[1],NETP[2],NETP[3],NETP[4]
Random Number Seed: 12399
Initial Values:     7,5,10,8,.7,3,3,3,3,7,7,7,7,3,1.2,1.2,1,1,1,1,15,15,15,15,500,30,1,7,10000,15000,20000,25000,350,400,450,500,50
...
void   costs(void);	/*** L[1],L[2],L[3],L[4],L1[1],L1[2],L1[3],L1[4],L2[1],L2[2],L2[3],L2[4],SERVICED[1],SERVICED[2],SERVICED[3],SERVICED[4],RUNC[1], ***/
...
double DERRICK;	/***  test  ***/
double THE;	/***  to help derrick  ***/
double ALPHA;	/***  Beta distribution alpha value  ***/
long   BETA;	/***  Beta distribution beta value  ***/
...
event_trace(const char * name_of_event,const long count)
{
  ...
  c_timest(INFLOW[1], 17, 0);
  c_sampst(INFLOW[1], 17, 0);
  c_timest(INFLOW[2], 18, 0);
  c_sampst(INFLOW[2], 18, 0);
  c_timest(INFLOW[3], 19, 0);
  c_sampst(INFLOW[3], 19, 0);
  c_timest(INFLOW[4], 20, 0);
  c_sampst(INFLOW[4], 20, 0);
  c_timest(CUMCOST[1], 21, 0);
  c_sampst(CUMCOST[1], 21, 0);
  c_timest(CUMCOST[2], 22, 0);
  c_sampst(CUMCOST[2], 22, 0);
  c_timest(CUMCOST[3], 23, 0);
  c_sampst(CUMCOST[3], 23, 0);
  c_timest(CUMCOST[4], 24, 0);
  c_sampst(CUMCOST[4], 24, 0);
  c_timest(NETP[1], 25, 0);
  c_sampst(NETP[1], 25, 0);
  c_timest(NETP[2], 26, 0);
  c_sampst(NETP[2], 26, 0);
  c_timest(NETP[3], 27, 0);
  c_sampst(NETP[3], 27, 0);
  c_timest(NETP[4], 28, 0);
  c_sampst(NETP[4], 28, 0);
  if(trace_flag) fprintf(output_fp, "%9.3f\t %6s\t%6d ",current_time,name_of_event,count);
  if(trace_flag) fprintf(output_fp, "	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g 	%7.3g \n"
,(double)L[1], (double)L[2], (double)L[3], (double)L[4], (double)L1[1], (double)L1[2], (double)L1[3], (double)L1[4], (double)L2[1], (double)L2[2], (double)L2[3], (double)L2[4], (double)SERVICED[1], (double)SERVICED[2], (double)SERVICED[3], (double)SERVICED[4], (double)INFLOW[1], (double)INFLOW[2], (double)INFLOW[3], (double)INFLOW[4], (double)CUMCOST[1], (double)CUMCOST[2], (double)CUMCOST[3], (double)CUMCOST[4], (double)NETP[1], (double)NETP[2], (double)NETP[3], (double)NETP[4]);
}
...
char y_n = 'p';            /* yes/no for file overwrite*/
  ...
  if (fscanf(input_fp,"%s %1s %ld %lf %d", output_file_name, &y_n, &rndsd, &stop_time, &trace_flag)<4
     ...
     || fscanf(input_fp,"%lf", &ALPHA)<1
     || fscanf(input_fp,"%ld", &BETA)<1
...
if (y_n == 'n' || y_n == 'N') {
  ...
  if (trace_flag != 0 && trace_flag != 1) {
    ...
    printf("Not found, input data at the keyboard.\n");
     ...
     scanf  ( "%lf", &RES);
     printf ( "\nEnter initial value for ALPHA: \n");
     scanf  ( "%lf", &ALPHA);
     printf ( "\nEnter initial value for BETA: \n");
     scanf  ( "%ld", &BETA);
...
if (trace_flag)
   ...
   fprintf(output_fp,"	       INFLOW[1]");
   fprintf(output_fp,"	       INFLOW[2]");
   fprintf(output_fp,"	       INFLOW[3]");
   fprintf(output_fp,"	       INFLOW[4]");
   fprintf(output_fp,"	      CUMCOST[1]");
   fprintf(output_fp,"	      CUMCOST[2]");
   fprintf(output_fp,"	      CUMCOST[3]");
   fprintf(output_fp,"	      CUMCOST[4]");
   fprintf(output_fp,"	         NETP[1]");
   fprintf(output_fp,"	         NETP[2]");
   fprintf(output_fp,"	         NETP[3]");
   fprintf(output_fp,"	           NETP[4] ");
   fprintf(output_fp,"\n");
   }
...
run_end()
{
  ...
  c_sampst(SERVICED[4], 16, 1);
   ...
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(INFLOW[1], 17, 1);
   fprintf(output_fp, "INFLOW[1]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("INFLOW[1]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(INFLOW[1], 17, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(INFLOW[2], 18, 1);
   fprintf(output_fp, "INFLOW[2]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("INFLOW[2]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(INFLOW[2], 18, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(INFLOW[3], 19, 1);
   fprintf(output_fp, "INFLOW[3]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("INFLOW[3]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(INFLOW[3], 19, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(INFLOW[4], 20, 1);
   fprintf(output_fp, "INFLOW[4]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("INFLOW[4]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(INFLOW[4], 20, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(CUMCOST[1], 21, 1);
   fprintf(output_fp, "CUMCOST[1]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("CUMCOST[1]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(CUMCOST[1], 21, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(CUMCOST[2], 22, 1);
   fprintf(output_fp, "CUMCOST[2]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("CUMCOST[2]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(CUMCOST[2], 22, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(CUMCOST[3], 23, 1);
   fprintf(output_fp, "CUMCOST[3]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("CUMCOST[3]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(CUMCOST[3], 23, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(CUMCOST[4], 24, 1);
   fprintf(output_fp, "CUMCOST[4]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("CUMCOST[4]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(CUMCOST[4], 24, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(NETP[1], 25, 1);
   fprintf(output_fp, "NETP[1]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("NETP[1]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(NETP[1], 25, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(NETP[2], 26, 1);
   fprintf(output_fp, "NETP[2]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("NETP[2]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(NETP[2], 26, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(NETP[3], 27, 1);
   fprintf(output_fp, "NETP[3]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("NETP[3]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(NETP[3], 27, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
   printf(" Minimum  = 	%7.4g\n", transfer[7]);
   printf(" Maximum  = 	%7.4g\n", transfer[6]);
  c_timest(NETP[4], 28, 1);
   fprintf(output_fp, "NETP[4]:\n Time Ave. = \t%7.4g Time Sample Var. =\t%7.4g\n", transfer[4], transfer[7]);
   printf("NETP[4]:\n Time Ave. = 	%7.4g Time Sample Var. = 	%7.4g\n", transfer[4], transfer[7]);
  c_sampst(NETP[4], 28, 1);
   fprintf(output_fp, " Event Ave. =\t%7.4g Event Sample Var. =\t%7.4g\n", transfer[4], transfer[8]);
   fprintf(output_fp, " Minimum =\t%7.4g\n", transfer[7]);
   fprintf(output_fp, " Maximum =\t%7.4g\n", transfer[6]);
   printf(" Event Ave.  = 	%7.4g Event Sample Var.= 	%7.4g\n", transfer[4], transfer[8]);
...
int  _edge_condition[4];
  ...
  if (_edge_condition[0])
    ...
    event_time = current_time + (1/ARRMM[J][1])+RND*(1/ARRMM[J][0]);
...
int  _edge_condition[2];
  ...
  P=RND;
  ENT[2]=CLK;
  THE=TF*2*RND-ENT[3]*.1;
  DERRICK=(THE<=0)+(THE>0)*THE;
  ...
  if (_edge_condition[0])
    ...
    event_time = current_time + REN[J][0]+(REN[J][1]-REN[J][0])*BET(ALPHA,BETA);
  ...
  if (_edge_condition[1])
    ...
    event_time = current_time + DERRICK;
...
int  _edge_condition[1];
  ...
  /* Evaluate edge conditions now so that they will*/
  /* not be changed by preemptive event execution  */
  _edge_condition[0] = ( ENT[3]/5000+ENT[4]>3.3&&S[J]>0 );
...
/*** L[1],L[2],L[3],L[4],L1[1],L1[2],L1[3],L1[4],L2[1],L2[2],L2[3],L2[4],SERVICED[1],SERVICED[2],SERVICED[3],SERVICED[4],RUNC[1], ***/
void
costs()
{
int  _edge_condition[1];
